...
export default {
  ...
  computed: {
    ...
    currentValue: {
      ...
      set(value) {
        this.isGrouped?this.radioGroup.$emit("input",value):this.$emit("input", value);
      },
    },
    radioClass(){
  ...
  methods:{
    handleChange(){
      this.isGrouped?this.radioGroup.$emit("change",this.value):this.$emit("change", this.value);
    }
  }
};
</script>
